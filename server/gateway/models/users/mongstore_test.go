...
func TestMongoStore(t *testing.T) {
	...
	if err != nil {
		t.Errorf("expecting error when inserting invalid new user: %v", err)
	}
	user, err = store.UpdateNotifications(updates, user.ID)
	if err != nil {
		t.Errorf("error updating notifications: %v", err)
	}
