...
docker run -d -p 443:443 --name milestoneapi --network milestonenetwork -v /etc/letsencrypt:/etc/letsencrypt:ro -e TLSCERT=/etc/letsencrypt/live/milestoneapi.eric-jacobson.me/fullchain.pem -e TLSKEY=/etc/letsencrypt/live/milestoneapi.eric-jacobson.me/privkey.pem -e REDISADDR=milestone-redis:6379 -e DBADDR=milestone-mongo:27017 -e SESSIONKEY=production ejacobson96/milestoneapi